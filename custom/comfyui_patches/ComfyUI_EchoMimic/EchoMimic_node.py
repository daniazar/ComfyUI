...
class Echo_Predata:
    ...
    def INPUT_TYPES(cls):
        ...
        return {
            "required": {
                ...
                "length": ("INT", {"default": 0, "min": 0, "max": 5000, "step": 1, "display": "number"}),
                "partial_video_length": ("INT", {"default": 65, "min": 5, "max": 1000, "step": 4, "display": "number"}),
                "overlap_video_length": ("INT", {"default": 8, "min": 0, "max": 100, "step": 1, "display": "number"}),
                "draw_mouse": ("BOOLEAN", {"default": False},),
                "motion_sync_": ("BOOLEAN", {"default": False},),
                "multi_camera_shots": ("BOOLEAN", {"default": True},),
                },
            "optional": {
                "clip": ("CLIP",),
    ...
    def main_loader(self, info, image,audio,prompt,negative_prompt,pose_dir,width,height,fps,facemask_ratio,facecrop_ratio,length,partial_video_length,overlap_video_length,draw_mouse,motion_sync_,multi_camera_shots=True,**kwargs):
        ...
        sf.write(buff, audio["waveform"].squeeze(0).cpu().numpy().T, audio["sample_rate"], format="FLAC")
        with open(audio_file, 'wb') as f:
            f.write(buff.getbuffer())

        if length == 0:
            length = int((audio["waveform"].shape[-1] / audio["sample_rate"]) * fps)
            print(f"Length set to 0. Automatically calculated video length: {length} frames.")
        ...
        if "V1"==version :
            ...
            emb["partial_video_length"] = int(partial_video_length)
            emb["overlap_video_length"] = int(overlap_video_length)
        else:
...
class Echo_Sampler:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "save_parts": ("BOOLEAN", {"default": False},), },    
        }
    ...
    def em_main(self,model, emb, seed, cfg, steps, sample_rate,context_frames, context_overlap,save_video,save_parts=False):
        ...
        if version=="V1":
            ...
            if version=="V3_flash":
                output_video=infer_flash(model,emb.get("audio_embeds"),emb.get("prompt_embeds"),emb.get("negative_prompt_embeds"),emb.get("clip_context"),
                                         ...
                                         emb.get("partial_video_length"), emb.get("overlap_video_length"), emb.get("ref_image_pil"),
                                         emb.get("temporal_compression_ratio"), emb.get("shot_boundaries"), save_parts
                                         )
...
class Echo_VideoCombine:
    @classmethod
    def INPUT_TYPES(s):
        import nodes
        VideoCombine = nodes.NODE_CLASS_MAPPINGS.get("VHS_VideoCombine", None)
        if VideoCombine is None:
            return {"required": {"images": ("IMAGE",)}}
            
        inputs = VideoCombine.INPUT_TYPES()
        if "pingpong" in inputs["required"]:
            del inputs["required"]["pingpong"]
    ...
    def combine_video(self, **kwargs):
        ...
        # We dynamically remove the pingpong code from the execution path
        # by temporarily monkey-patching the VHS to_pingpong function to do nothing.
        import sys
        vhs_nodes = None
        for mod_name, mod in sys.modules.items():
            if "videohelpersuite.nodes" in mod_name and hasattr(mod, "to_pingpong"):
                vhs_nodes = mod
                break
                
        original_to_pingpong = None
        if vhs_nodes:
            original_to_pingpong = vhs_nodes.to_pingpong
            vhs_nodes.to_pingpong = lambda x: x  # Replace with a no-op that just returns the original images

        try:
            vc = VideoCombine()
            return vc.combine_video(**kwargs)
        finally:
            if vhs_nodes and original_to_pingpong:
                vhs_nodes.to_pingpong = original_to_pingpong
...
NODE_CLASS_MAPPINGS = {
    ...
    "Echo_VideoCombine": Echo_VideoCombine,
}
...
NODE_DISPLAY_NAME_MAPPINGS = {
    ...
    "Echo_Predata": "Echo_Predata",
    "Echo_Sampler": "Echo_Sampler",
    "Echo_VideoCombine": "Echo_VideoCombine (No PingPong)",
}
